...
function optimizeBlock(statements) {
  ...
  for (const statement of statements) {
    ...
    const flattened = Array.isArray(optimizedStmt) ? optimizedStmt : [optimizedStmt]
    result.push(...flattened)

    if (
      flattened.some(s =>
        ["CutStatement", "ReturnStatement", "ShortReturnStatement"].includes(s.kind),
      )
    ) {
      break
    }
...
const optimizers = {
  ...
  MeasureRangeStatement(s) {
    ...
    if (s.low.constructor === Number && s.high.constructor === Number && s.low > s.high)
      return []
    return s
  },
  MeasureInStatement(s) {
    ...
    if (s.collection?.kind === "ArrayLiteral" && s.collection.elements.length === 0)
      return []
    return s
  },
