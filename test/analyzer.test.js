...
const validControlFlow = [
  ...
  ["cue-alt statement without drop", "cue open : play 1 alt closed : play 2 cadence"],
  [
...
const validFunctions = [
  ...
  ["parent scope access", "note x = 1 compose f() -> level : fin x cadence play f()"],
...
const invalidScopeAndMutability = [
  ...
  ["redeclaration of variable", "note x = 1 note x = 2", /Identifier already declared/],
  ["assign to undeclared variable", "x = 1", /Undefined identifier/],
  ["overwrite immutable key", "key x = 1 x = 2", /Cannot overwrite immutable 'key'/],
  ["bump immutable key", "key x = 1 x sharp", /Cannot overwrite immutable 'key'/],
...
const invalidControlFlowAndLoops = [
  ...
  ["non-level count in encore", "encore open : play 1 cadence", /Expected a level/],
  [
...
const invalidTypesAndExpressions = [
  ["type mismatch assigning gate to level", "note x = 1 x = open", /Type mismatch/],
  ["type mismatch assigning level to gate", "note x = open x = 1", /Type mismatch/],
...
const invalidArraysAndOptionals = [
  ...
  ["unwrap non-optional", "note x = 5 play x ?? 10", /Can only unwrap a 'ghost' type/],
  [
    "unwrap type mismatch",
    "note x = ghost 5 play x ?? open",
    /Unwrap types do not match/,
  ],
  ["subscript non-array", "note x = 1 play x[0]", /Can only subscript an array/],
  ["subscript with non-level", "note arr = [1] play arr[open]", /Expected a level/],
]
...
const invalidFunctionsAndStructs = [
  ...
  ["member access on non-struct", "note x = 1 play x.y", /Target is not a chord/],
  [
    "member access unknown field 1",
    "chord Point : x : level cadence note pt = 1 play pt.z",
    /Target is not a chord/,
  ],
  ["member access on array", "note arr = [1] play arr.x", /Target is not a chord/],
  [
    "member access unknown field 2",
    "chord Point : x : level cadence compose f(pt : Point) : play pt.z cadence",
    /has no note named z/,
  ],
  ["compare function to level", "compose f() : cadence play f == 1", /Type mismatch/],
]
